...
public class MongoPluginDataTypeTest {
    ...
    public void testBsonTypes() {
        ...
        assertEquals(DataType.BSON, DataTypeServiceUtils.getAppsmithType(ClientDataType.OBJECT,"{}{}", pluginSpecificTypes).type());
        assertEquals(DataType.BSON, DataTypeServiceUtils.getAppsmithType(ClientDataType.OBJECT,"{}[]null", pluginSpecificTypes).type());
        // Numbers may be NaNs or infinities.
        assertEquals(DataType.BSON, DataTypeServiceUtils.getAppsmithType(ClientDataType.OBJECT,"{\"number\": NaN}", pluginSpecificTypes).type());
        assertEquals(DataType.BSON, DataTypeServiceUtils.getAppsmithType(ClientDataType.OBJECT,"{\"number\": Infinity}", pluginSpecificTypes).type());
        // Names that are unquoted or 'single quoted'.
        assertEquals(DataType.BSON, DataTypeServiceUtils.getAppsmithType(ClientDataType.OBJECT,"{a: 1}", pluginSpecificTypes).type());
        assertEquals(DataType.BSON, DataTypeServiceUtils.getAppsmithType(ClientDataType.OBJECT,"{'a': 1}", pluginSpecificTypes).type());
        // Strings that are unquoted or 'single quoted'.
        assertEquals(DataType.BSON, DataTypeServiceUtils.getAppsmithType(ClientDataType.OBJECT,"{\"a\": ''}", pluginSpecificTypes).type());
        // Unnecessary array separators. These are interpreted as if null was the omitted value.
        assertEquals(DataType.BSON, DataTypeServiceUtils.getAppsmithType(ClientDataType.OBJECT, "{\"a\": [1,]}", pluginSpecificTypes).type());

        assertEquals(DataType.BSON, DataTypeServiceUtils.getAppsmithType(ClientDataType.OBJECT,"{\"a\": 0,}", pluginSpecificTypes).type());
    }
}
