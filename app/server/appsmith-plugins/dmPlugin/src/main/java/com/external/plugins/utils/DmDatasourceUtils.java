...
public class DmDatasourceUtils {
     ...
     * +------------+-----------+-----------------+-----------------+-------------------+
     */
    public static final String DM_SQL_QUERY_TO_GET_ALL_TABLE_COLUMN_KEY_CONSTRAINTS =
            "SELECT " +
            "    cols.table_name," +
            "    cols.column_name," +
        ...
        return Mono.fromSupplier(() -> {
                    ...
                    try (Statement statement = connectionFromPool.createStatement()) {
                        ...
                        return Mono.error(new AppsmithPluginException( AppsmithPluginError.PLUGIN_GET_STRUCTURE_ERROR,
                                DmErrorMessages.GET_STRUCTURE_ERROR_MSG, throwable.getCause(),
                                "SQLSTATE: " + throwable.getSQLState()));
                    } finally {
                        logHikariCPStatus("After getting DM DB schema", connectionPool);
                        safelyCloseSingleConnectionFromHikariCP(connectionFromPool, "Error returning DM connection to pool " +
    ...
    private static void setTableNamesAndColumnNamesAndColumnTypes(Statement statement, Map<String,
            DatasourceStructure.Table> tableNameToTableMap) throws SQLException {
        try (ResultSet columnsResultSet =
                     statement.executeQuery(DM_SQL_QUERY_TO_GET_ALL_TABLE_COLUMN_TYPE)) {
            while (columnsResultSet.next()) {
                final String tableName = columnsResultSet.getString("TABLE_NAME");
    ...
    public static HikariDataSource createConnectionPool(DatasourceConfiguration datasourceConfiguration) throws AppsmithPluginException {
        ...
        if (datasourceConfiguration.getConnection() == null
                || datasourceConfiguration.getConnection().getSsl() == null
                || datasourceConfiguration.getConnection().getSsl().getAuthType() == null) {
            throw new AppsmithPluginException(DmPluginError.DM_PLUGIN_ERROR,
                    DmErrorMessages.SSL_CONFIGURATION_ERROR_MSG);
        }
        ...
        try {
            ...
            throw new AppsmithPluginException(AppsmithPluginError.PLUGIN_DATASOURCE_ARGUMENT_ERROR,
                    DmErrorMessages.CONNECTION_POOL_CREATION_FAILED_ERROR_MSG, e.getMessage());
        }
