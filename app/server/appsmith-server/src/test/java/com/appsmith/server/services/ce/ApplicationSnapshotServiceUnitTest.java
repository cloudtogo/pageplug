...
public class ApplicationSnapshotServiceUnitTest {
        ...
        public void createApplicationSnapshot_WhenApplicationTooLarge_SnapshotCreatedSuccessfully() {
                ...
                Mockito.when(applicationService.findBranchedApplicationId(branchName, defaultAppId,
                                AclPermission.MANAGE_APPLICATIONS))
                                .thenReturn(Mono.just(branchedAppId));

                Mockito.when(importExportApplicationService.exportApplicationById(branchedAppId,
                ...
                // we're expecting to receive two application snapshots, create a matcher to
                // check the size
                ArgumentMatcher<List<ApplicationSnapshot>> snapshotListHasTwoSnapshot = snapshotList -> snapshotList
                                .size() == 2;
        ...
        public void restoreSnapshot_WhenSnapshotHasMultipleChunks_RestoredSuccessfully() {
                ...
                Mockito.when(applicationService.findByBranchNameAndDefaultApplicationId(branch, defaultAppId,
                                AclPermission.MANAGE_APPLICATIONS))
                ...
                List<ApplicationSnapshot> snapshots = List.of(
                                createSnapshot(branchedAppId,
                                                copyOfRange(jsonStringBytes, chunkSize * 2, jsonStringBytes.length), 3),
                                createSnapshot(branchedAppId, copyOfRange(jsonStringBytes, 0, chunkSize), 1),
                                createSnapshot(branchedAppId, copyOfRange(jsonStringBytes, chunkSize, chunkSize * 2),
                                                2));

                Mockito.when(applicationSnapshotRepository.findByApplicationId(branchedAppId))
                                .thenReturn(Flux.fromIterable(snapshots));

                // matcher to check that ApplicationJson created from chunks matches the
                // original one
                ArgumentMatcher<ApplicationJson> matchApplicationJson;
                matchApplicationJson = applicationJson1 -> applicationJson1.getExportedApplication().getName()
                                .equals(application.getName());
                Mockito.when(importExportApplicationService.importApplicationInWorkspace(
                                eq(application.getWorkspaceId()), argThat(matchApplicationJson), eq(branchedAppId),
                                eq(branch)))
                                .thenReturn(Mono.just(application));
