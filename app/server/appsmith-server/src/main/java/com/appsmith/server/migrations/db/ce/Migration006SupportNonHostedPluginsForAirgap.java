...
public class Migration006SupportNonHostedPluginsForAirgap {
    ...
    public void supportNonHostedPluginsForAirgap() {
        ...
        final Set<String> supportedNonSelfHostedPluginPackageNameForAirgap = Set.of(
                FIRESTORE_PLUGIN, REDSHIFT_PLUGIN, DYNAMO_PLUGIN, AMAZON_S3_PLUGIN
        );
        ...
        for (Plugin plugin : plugins) {
            if (supportedNonSelfHostedPluginPackageNameForAirgap.contains(plugin.getPackageName())) {
                ...
                mongoTemplate.updateFirst(
                        new Query().addCriteria(Criteria.where(FieldName.ID).is(plugin.getId())),
                        update,
                        Plugin.class
                );
            }
