...
public class GitFileUtils {
    ...
    private static final Set<String> blockedMetadataFields = Set.of(EXPORTED_APPLICATION, DATASOURCE_LIST, PAGE_LIST,
            ACTION_LIST, ACTION_COLLECTION_LIST,
     ...
     * @param applicationJson application reference object from which entire
    ...
    public Mono<Path> saveApplicationToLocalRepo(Path baseRepoSuffix,
        ...
        try {
            Mono<Path> repoPathMono = fileUtils
                    .saveApplicationToGitRepo(baseRepoSuffix, applicationReference, branchName).cache();
            return Mono.zip(repoPathMono, sessionUserService.getCurrentUser())
                    .flatMap(tuple -> {
                        ...
                        return analyticsService
                                .sendEvent(AnalyticsEvents.UNIT_EXECUTION_TIME.getEventName(),
    ...
    /**
     * Method to convert application resources to the structure which can be
     * serialised by appsmith-git module for
     * serialisation
     *
     * @param applicationJson application resource including actions, jsobjects,
     *                        pages
    ...
    public ApplicationGitReference createApplicationReference(ApplicationJson applicationJson) {
        ...
        // No need to commit publish mode theme as it leads to conflict resolution at
        // both the places if any
        ...
        // Insert only active pages which will then be committed to repo as individual
        // file
        ...
        applicationJson
                ...
                // assume if the unpublished version is deleted entity should not be committed
                // to git
        ...
        // Insert active actions and also assign the keys which later will be used for
        // saving the resource in actual filepath
        ...
        applicationJson
                ...
                .forEach(newAction -> {
                    String prefix = newAction.getUnpublishedAction() != null
                            ? newAction.getUnpublishedAction().getValidName() + NAME_SEPARATOR
                    ...
                    String body = newAction.getUnpublishedAction().getActionConfiguration().getBody() != null
                            ? newAction.getUnpublishedAction().getActionConfiguration().getBody()
                            : "";

                    // This is a special case where we are handling REMOTE type plugins based
                    // actions such as Twilio
                    // The user configured values are stored in a attribute called formData which is
                    // a map unlike the body
                    if (newAction.getPluginType().toString().equals("REMOTE")
                        ...
                        body = new Gson().toJson(
                                newAction.getUnpublishedAction().getActionConfiguration().getFormData(), Map.class);
                        newAction.getUnpublishedAction().getActionConfiguration().setFormData(null);
                    }
                    // This is a special case where we are handling JS actions as we don't want to
                    // commit the body of JS actions
        ...
        // Insert JSOObjects and also assign the keys which later will be used for
        // saving the resource in actual filepath
        ...
        applicationJson
                ...
                .forEach(actionCollection -> {
                    String prefix = actionCollection.getUnpublishedCollection() != null
                            ? actionCollection.getUnpublishedCollection().getName() + NAME_SEPARATOR
                                    + actionCollection.getUnpublishedCollection().getPageId()
                            : actionCollection.getPublishedCollection().getName() + NAME_SEPARATOR
     ...
     *
     * @param workspaceId          To which workspace application needs to be
     *                             rehydrated
     * @param defaultApplicationId Root application for the current branched
     *                             application
     * @param branchName           for which branch the application needs to
     *                             rehydrate
    ...
    public Mono<ApplicationJson> reconstructApplicationJsonFromGitRepo(String workspaceId,
        ...
        return Mono.zip(appReferenceMono, sessionUserService.getCurrentUser())
                .flatMap(tuple -> {
                    ...
                    ApplicationJson metadata = getApplicationResource(applicationReference.getMetadata(),
                    ...
                    final Map<String, Object> data = Map.of(
                            ...
                            "executionTime", stopwatch.getExecutionTime());
                    return analyticsService
                            .sendEvent(AnalyticsEvents.UNIT_EXECUTION_TIME.getEventName(), tuple.getT2().getUsername(),
                                    data)
    ...
    /**
     * Once the user connects the existing application to a remote repo, we will
     * initialize the repo with Readme.md -
     ...
     * Welcome message
     *
     * @param baseRepoSuffix path suffix used to create a branch repo path as per
    ...
    /**
     * When the user clicks on detach remote, we need to remove the repo from the
     * file system
     *
     * @param baseRepoSuffix path suffix used to create a branch repo path as per
     *                       worktree implementation
    ...
    private void removeUnwantedFieldsFromPage(NewPage page) {
        ...
        // published and unpublished PageDTO will
    ...
    private void removeUnwantedFieldsFromApplication(Application application) {
        // Don't commit application name as while importing we are using the repoName as
        // application name
    ...
    private void removeUnwantedFieldFromAction(NewAction action) {
        // As we are publishing the app and then committing to git we expect the
        // published and unpublished ActionDTO will
    ...
    private void removeUnwantedFieldFromActionCollection(ActionCollection actionCollection) {
        ...
        // ActionCollectionDTO will be same, so we only commit unpublished
        // ActionCollectionDTO.
    ...
    private ApplicationJson getApplicationJsonFromGitReference(ApplicationGitReference applicationReference) {
        ...
        // Clone the edit mode theme to published theme as both should be same for git
        // connected application because we
        ...
        if (application != null && !CollectionUtils.isNullOrEmpty(application.getPages())) {
            // Remove null values
            org.apache.commons.collections.CollectionUtils.filter(application.getPages(),
            ...
            application.getPages()
                    .forEach(applicationPage -> applicationPages
                            .add(gson.fromJson(gson.toJson(applicationPage), ApplicationPage.class)));
        ...
        if (CollectionUtils.isNullOrEmpty(applicationReference.getActions())) {
            ...
            actions.forEach(newAction -> {
                // With the file version v4 we have split the actions and metadata separately
                // into two files
                ...
                if (actionBody != null && (actionBody.containsKey(keyName))
                    ...
                    // For REMOTE plugin like Twilio the user actions are stored in key value pairs
                    // and hence they need to be
                ...
                // As we are publishing the app and then committing to git we expect the
                // published and unpublished
                // actionDTO will be same, so we create a deep copy for the published version
                // for action from
        ...
        if (CollectionUtils.isNullOrEmpty(applicationReference.getActionCollections())) {
            ...
            List<ActionCollection> actionCollections = getApplicationResource(
            ...
            actionCollections.forEach(actionCollection -> {
                ...
                // As we are publishing the app and then committing to git we expect the
                // published and unpublished
                // actionCollectionDTO will be same, so we create a deep copy for the published
                // version for
                // actionCollection from unpublishedActionCollectionDTO
                actionCollection.setPublishedCollection(gson
                        .fromJson(gson.toJson(actionCollection.getUnpublishedCollection()), ActionCollectionDTO.class));
            });
        ...
        // Extract datasources
        applicationJson
                .setDatasourceList(getApplicationResource(applicationReference.getDatasources(), Datasource.class));
