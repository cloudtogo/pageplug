...
public class UserControllerCE extends BaseController<UserService, User, String> {
    ...
    public Mono<ResponseDTO<UserProfileDTO>> getUserProfile(
            @CookieValue(name = "token", defaultValue = "null") String tokenInCloudOS) {
        final Boolean inCloudOS = cloudOSConfig.getInCloudOS();
        if (inCloudOS) {
            return cloudOSActionSolution.verifyCloudOSToken(tokenInCloudOS).flatMap(valid -> sessionUserService
                    .getCurrentUser()
                    .map(user -> {
