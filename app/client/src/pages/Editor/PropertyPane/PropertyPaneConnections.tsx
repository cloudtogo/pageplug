...
const TriggerNode = (props: TriggerNodeProps) => {
  const ENTITY = props.entityCount > 1 ? "多个实体" : "单个实体";
  const tooltipText = !!props.entityCount
    ? `查看 ${props.connectionType.toLowerCase()} 连接`
    : `暂无 ${props.connectionType.toLowerCase()} 连接`;
    ...
    <TopLayer ref={topLayerRef}>
      <Menu>
        ...
        <MenuContent align="start" style={{ width: "250px" }}>
          <MenuItem className="menuitem-nohover">输入连接</MenuItem>
          <MenuSeparator />
      ...
      <Menu>
        ...
        <MenuContent align="end" style={{ width: "250px" }}>
          <MenuItem className="menuitem-nohover">输出连接</MenuItem>
          <MenuSeparator />
