import { cancelled, delay, put, take, spawn } from "redux-saga/effects";
...
export class GracefulWorkerService {
  ...
  private _evaluationWorker: Taro.Worker | undefined;
  ...
  *start() {
    ...
    this._evaluationWorker = Taro.createWorker("worker/worker.js");
    this._evaluationWorker.onMessage(this._broker);
  ...
  *duplexResponseHandler(
  ...
  private _broker(result: any) {
    const { requestId, responseData, timeTaken } = result;
