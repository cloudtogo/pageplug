...
export const VALIDATORS: Record<ValidationTypes, Validator> = {
  ...
  [ValidationTypes.DATE_ISO_STRING]: (
    ...
    const invalidResponse = {
      isValid: false,
      parsed: config.params?.default || dayjs().toISOString(),
    ...
    if (isString(value)) {
      if (!dayjs(value).isValid()) return invalidResponse;

      if (value === dayjs(value).toISOString()) {
        return {
          isValid: true,
          parsed: value,
        };
      ...
      if (dayjs(value).isValid())
        return { isValid: true, parsed: dayjs(value).toISOString() };
    }
    return invalidResponse;
  },
