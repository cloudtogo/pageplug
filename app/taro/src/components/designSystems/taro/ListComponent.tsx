...
const freeSize = (n: string) => {
  if (n.toLowerCase().endsWith("px")) {
    return transformDynamicSize(parseInt(n));
  } else {
    return n;
  }
};
...
const FreeImage = styled(Image)<{
  ...
  width: ${(props) => freeSize(props.width)};
  height: ${(props) => freeSize(props.height)};
  pointer-events: none;
`;
...
const ListComponent = (props: ListComponentProps) => {
  ...
  const makeSwipe = (content: React.ReactNode, item: any, index: number) => {
    return (
      <SwipeCellContainer key={index + `${item?.id}`}>
        <SwipeCell onClick={onClickItem(item)}>
          {content}
